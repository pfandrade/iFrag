...
//	2005-07-14	- added disabled look
//	2005-11-15	- replaced nested function showMenu by method
//	2006-01-07	- send action method from control instead of from cell
//				- support NSOnOffButton button type
//	2006-01-12	- fixed menu arrow position
//	2006-04-17	- -drawWithFrame:inView: honors button state (on/off)
//	2006-11-17	- -widthForFrame: should return more sensible values now
...
- (void)finishInit
{
	...
	[self setAmBezelStyle:NSRoundedBezelStyle];
...
- (id)initWithCoder:(NSCoder *)decoder
{
	...
	[decoder decodeValueOfObjCType:@encode(int) at:&am_buttonType];
	[self setAmBezelStyle:[super bezelStyle]];
	[super setBezelStyle:NSShadowlessSquareBezelStyle];
	[self setAlignment:NSCenterTextAlignment];
	return self;
}
...
{
	[super setBezelStyle:[self amBezelStyle]];
	...
	[coder encodeValueOfObjCType:@encode(int) at:&am_buttonType];
}
...
{
	return am_buttonType;
}

- (void)setButtonType:(NSButtonType)newButtonType
{
	am_buttonType = newButtonType;
	[super setButtonType:newButtonType];
}
...
- (NSBezelStyle)amBezelStyle
{
	return am_bezelStyle;
}

- (void)setAmBezelStyle:(NSBezelStyle)newAmBezelStyle
{
	//[super setBezelStyle:newBezelStyle];
	am_bezelStyle = newAmBezelStyle;
...
- (void)calculateLayoutForFrame:(NSRect)cellFrame inView:(NSView *)controlView
{
	...
	float textInset;
	if ([self amBezelStyle] != NSShadowlessSquareBezelStyle) {
		float h = [font ascender]/2.0;
		textInset = ceilf(radius-sqrt(radius*radius - h*h));
		//NSLog(@"h: %f, radius: %f, inset: %f", h, radius, textInset);
	} else {
		textInset = ceilf([font ascender]-([font fixed_xHeight]/2.0)); //*am_textInsetFactor;
	}
	...
	if ([self menu]) {
		...
		//float x = am_lastFrameSize.width-am_backgroundInset-am_textRect.origin.x-arrowWidth;
		float x = am_lastFrameSize.width-am_backgroundInset-(radius*0.5)-arrowWidth;
	...
	//NSLog(@"text inset: = %f", am_textRect.origin.x-cellFrame.origin.x);
}

- (void)drawWithFrame:(NSRect)cellFrame inView:(NSView *)controlView
{
	//NSLog(@"drawWithFrame: %f, %f", cellFrame.size.width, cellFrame.size.height);
	...
	if ([self isBordered]) {
		if ([self state] == NSOnState) {
			...
			if ([self isEnabled]) {
				...
				[[am_frameColor disabledColor] set];
			}
		}
	...
	if ([self isHighlighted]) {
		...
		if (am_shadingColor) {
			...
			if ([self isEnabled]) {
				...
				shadingStartColor = [am_shadingColor disabledColor];
				shadingEndColor = [am_controlColor disabledColor];
			}
		}
		[am_controlColor set];
		path = [[am_backgroundPath copy] autorelease];
		if ([self isEnabled]) {
			...
			textColor = [am_textColor disabledColor];
			arrowColor = [am_arrowColor disabledColor];
		}
	...
	while (!done) { // loop ...
		...
		if (event) { // mouse event
			...
			if (done && result && ![self isContinuous]) {
				[NSApp sendAction:[self action] to:[self target] from:controlView];
				if ([self buttonType] == NSOnOffButton) {
					[self setNextState];
				}
			}
		} else { // show menu
