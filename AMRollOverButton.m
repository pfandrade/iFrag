...
//	2005-07-18	- register for NSWindowDidResizeNotification
...
{
	NSRect cellRect = [self frame];
	//NSLog(@"frame size: %f %f", cellRect.size.width, cellRect.size.height);
	cellRect.origin = NSZeroPoint;
	cellRect = NSInsetRect(cellRect, am_insetCell, am_insetCell);
	float width = [[self cell] widthForFrame:cellRect];
	NSRect newFrame = [self frame];
	newFrame.size.width = width;
	//NSLog(@"new frame size: %f %f", newFrame.size.width, newFrame.size.height);
